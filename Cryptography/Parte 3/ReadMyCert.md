# Objetivo
How about we take you on an adventure on exploring certificate signing requests Take a look at this CSR file [here](https://artifacts.picoctf.net/c/422/readmycert.csr).

# Solucion
```Shell

┌──(kali㉿kali)-[~/Downloads]
└─$ cat readmycert.csr
-----BEGIN CERTIFICATE REQUEST-----

-----END CERTIFICATE REQUEST-----

```
vamos a [CSR Decoder - Check CSR to verify its contents (sslshopper.com)](https://www.sslshopper.com/csr-decoder.html) metemos el texto que nos dio el cat y nos dara la bandera  
##### picoCTF{read_mycert_373b4ab0}
# Notas adicionales

# Referencias